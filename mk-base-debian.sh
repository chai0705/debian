...
case "$TARGET" in
    xfce)
        apt install -y task-xfce-desktop blueman onboard rsyslog sudo dialog apt-utils ntp evtest udev
        repair_dpkg
        ;;
    lite)
...
DATE=$(date +%Y%m%d)
echo -e "\033[47;36m 运行 tar 打包 debian-base-$TARGET-$ARCH-$DATE.tar.xz \033[0m"
XZ_OPT=-T0 tar -cpJf debian-base-$TARGET-$ARCH-$DATE.tar.xz $TARGET_ROOTFS_DIR
