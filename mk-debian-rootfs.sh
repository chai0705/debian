...
select_target() {
    while true; do
        ...
        echo -e "\033[42;36m [0] 退出 \033[0m"
        echo -e "\033[42;36m [1] xfce \033[0m"
        echo -e "\033[42;36m [2] lite \033[0m"
        echo -e "\033[42;36m --------------------------------------------------------- \033[0m"
        read -p "选择: " input

        case $input in
            0) exit;;  # 退出
            1) TARGET=xfce; break;;  # 选择xfce
            2) TARGET=lite; break;;  # 选择lite
            *) echo -e "\033[42;36m 无效输入，请重试。 \033[0m";;  # 输入错误提示
        esac
    done
...
# 检查是否存在 Ubuntu 基础镜像，如果不存在则运行 mk-base-debian.sh 构建基础系统
if [ ! -e debian-base-"$TARGET"-$ARCH-*.tar.xz ]; then
    echo -e "\033[42;36m 未存在基础包，运行 mk-base-debian.sh 构建基本系统 \033[0m"
    source mk-base-debian.sh
fi
...
tar -xf debian-base-$TARGET-$ARCH-*.tar.xz  # 解压基础镜像
...
retry_command cp -rpf packages/$ARCH/$RGA/*.deb $TARGET_ROOTFS_DIR/packages/install_packages
